...
export default async function RiderCompletedPage() {
  ...
  const shipments = await prisma.shipment.findMany({
    where: { riderId: user.id, status: { in: ["DELIVERED", "FAILED"] } },
    orderBy: { deliveredAt: "desc" },
  });

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-navy-900">Completed Deliveries</h1>
      {shipments.length === 0 ? (
          ...
          {shipments.map((s) => (
            <Link key={s.id} href={`/dashboard/rider/shipments/${s.id}`} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50">
              <div>
                ...
                <p className="text-sm text-gray-500">{s.shipmentType} · {formatDate(s.deliveredAt ?? s.updatedAt)}</p>
              </div>
